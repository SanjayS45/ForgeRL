...
class GoalConditionedRewardModel(nn.Module):
    ...
    def forward(
        ...
        instr_feat = self.instr_proj(instr_emb)
        
        goal_feat = goal_feat.unsqueeze(1).expand(-1, T, -1)
        instr_feat = instr_feat.unsqueeze(1).expand(-1, T, -1)
        
        sa = torch.cat([obs, act], dim=-1)
        sa_feat = self.state_action_enc(sa)
        
        combined = torch.cat([sa_feat, goal_feat, instr_feat], dim=-1)
    ...
    def forward_step(
        ...
        return self.forward(obs, act, goal, instr_emb)
